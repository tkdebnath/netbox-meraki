...
For fresh installs, use this script to ensure clean database state:
...
python manage.py migrate netbox_meraki zero 2>/dev/null || true
pip uninstall -y netbox-meraki 2>/dev/null || true

echo "Fresh installation..."
cd /opt/custom_plugin/ra_netbox_meraki
pip install -e .

cd /opt/netbox/netbox
# Generate migration that matches current models
...
**Note**: The migration file is intentionally not included in the repository. It will be auto-generated by Django's `makemigrations` command to match your exact NetBox version and model definitions.
